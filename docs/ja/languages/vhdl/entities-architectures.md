...
VHDLでは、ハードウェアモジュールは外部入出力インターフェースを定義する**エンティティ**と、内部実装を定義する**アーキテクチャ**の2つの独立した宣言に厳密に分離されます。

---

## エンティティ宣言 (`entity`)

エンティティは、ジェネリックパラメータやポートピンを含むハードウェアモジュールの主要インターフェースを定義します:
...
## アーキテクチャ本体 (`architecture`)

アーキテクチャは、エンティティの内部動作または構造的ネットリストを記述します:
...
VHDLは、構造的コンポーネント宣言とポートマップによる階層設計をサポートしています:

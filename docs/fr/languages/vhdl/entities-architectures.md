...
En VHDL, les modules matériels sont rigoureusement séparés en deux déclarations distinctes : l'**Entité** (qui définit l'interface externe d'E/S) et l'**Architecture** (qui définit l'implémentation interne).

---

## Déclarations d'entité (`entity`)

L'entité définit l'interface principale du module matériel, y compris les paramètres génériques et les broches de port :
...
## Corps d'architecture (`architecture`)

L'architecture décrit le comportement interne ou la netlist structurelle de l'entité :
...
## Instanciation de composants structurels

VHDL prend en charge la conception hiérarchique via les déclarations de composants structurels et les mappages de ports (port maps) :

...
- [ ] **Phase 38: IEEE 1364 Value Change Dump (VCD) Import & Golden Model Silicon Waveform Diffing - [P3]**
  - [ ] **In-Engine VCD File Parser (`crates/telemetry/src/vcd_import.rs`)**:
    - High-speed streaming parser for standard `.vcd` files generated by external simulators or hardware logic analyzers.
  - [ ] **External Waveform Importer UI (`ImportVcdModal.tsx`)**:
    - Drag-and-drop file importer loading `.vcd` traces into the Waveform Viewer.
  - [ ] **Golden Model Silicon Waveform Diff Engine**:
    - Side-by-side comparison of Axiom simulation vs. golden VCD, automatically pinpointing and highlighting timestamped signal mismatches in crimson.
